...
const useProfileData = () => {
    ...
    const fetchOrderCountAPI = async () => {
        ...
        try {
            ...
            console.log(`✅ [DEBUG HOOK] /orders/count: Success, Count = ${count}`);
            return count;
        } catch (e) {
            const errorMsg = e.response?.data?.message || e.message || "Unknown error";
            console.error(`❌ [DEBUG HOOK] API Error /orders/count (Status: ${e.response?.status || 'N/A'}):`, errorMsg);
    ...
    const fetchAddressCountAPI = async () => {
        ...
        try {
            ...
            // Backend trả về { success: true, count: number }
            const count = response.data?.count || 0;
            console.log(`✅ [DEBUG HOOK] /addresses/count: Success, Count = ${count}`);
            return count;
        } catch (e) {
            const errorMsg = e.response?.data?.message || e.message || "Unknown error";
            console.error(`❌ [DEBUG HOOK] API Error /addresses/count (Status: ${e.response?.status || 'N/A'}):`, errorMsg);
            return 0;
        }
    };
    ...
    useEffect(() => {
        if (!user || !token) {
            console.log("[DEBUG HOOK] 🚫 User or Token not found. Skipping Load Counts.");
            setIsCounting(false);
            return;
        }

        console.log("[DEBUG HOOK] 🔄 Starting to load Profile Counts...");
        ...
        const loadAllCounts = async () => {
            ...
            console.log("[DEBUG HOOK] ✅ Load Counts Completed.");
            setIsCounting(false);
        };
